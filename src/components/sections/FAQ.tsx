...
export const FAQ = () => {
  ...
  return (
    <section id="faq" className="py-32 px-6 bg-brand-cream relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-brand-sage/10 rounded-full blur-3xl -ml-48 -mb-48" />
        ...
        <div className="space-y-12">
          <div className="space-y-6">
            ...
            <h2 className="text-5xl md:text-6xl font-serif font-bold text-brand-dark leading-tight">
              Sering <br /> <span className="text-brand-sage italic">Ditanyakan</span>
            </h2>
            ...
            {FAQ_ITEMS.map((item, idx) => (
              <motion.div 
                ...
                className={`group rounded-2xl overflow-hidden shadow-sm transition-all duration-300 ${
                  openIndex === idx 
                    ? 'bg-white shadow-xl ring-1 ring-brand-gold/20' 
              ...
              >
                ...
                >
                  <span className={`text-lg font-bold tracking-tight ${openIndex === idx ? 'text-brand-gold' : 'text-brand-dark group-hover:text-brand-sage'}`}>
                    {item.question}
                  </span>
                  <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${
                    openIndex === idx ? 'bg-brand-gold text-white rotate-180' : 'bg-brand-pink/30 text-brand-sage'
                  }`}>
